...
1. **Write path**: App calls `insert/update/delete` → `PulseDb.execute()` modifies SQLite → `ChangeNotifier.notify(tables)` → debounced stream emission → `watch()` subscribers emit new query results.

2. **Read path**: `query(sql)` runs immediately and returns `List<Map<String, dynamic>>`.

3. **Reactive path**: `watch(table)` returns a broadcast `Stream`. On first listener, it emits the current data immediately, then subscribes to `ChangeNotifier.changes` and re-emits whenever the tracked tables change.
...
| Decision | Rationale |
|----------|-----------|
| `ChangeNotifier` has a **50 ms debounce** | Multiple writes in the same microtask coalesce into one notification |
